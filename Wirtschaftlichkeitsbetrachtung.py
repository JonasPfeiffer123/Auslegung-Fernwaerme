...
def annuität(A0, TN, f_Inst, f_W_Insp, Bedienaufwand=0, q=1.05, r=1.03, T=20, Energiebedarf=0, Energiekosten=0, E1=0):
    ...
    A_N = A_NE - (A_N_K + A_N_V + A_N_B + A_N_S)  # Annuität

    A_N *= -1  # Annuität soll positiv sein, um Wärmegestehungskosten zu berechnen
...
def WGK_WP(Wärmeleistung, Wärmemenge, Strombedarf, Wärmequelle, spez_Investitionskosten_WQ, Strompreis, q, r, T, BEW="Nein"):
    ...
    Annuität_WP = annuität(Investitionskosten_WP, Nutzungsdauer_WP, f_Inst_WP, f_W_Insp_WP, Bedienaufwand_WP, q, r, T,
                           Strombedarf, Strompreis)
    WGK_WP_a = Annuität_WP/Wärmemenge

    spezifische_Investitionskosten_WQ_dict = {"Abwärme": 500, "Abwasserwärme": 1000, "Geothermie": spez_Investitionskosten_WQ}
    ...
    Annuität_WQ = annuität(Investitionskosten_WQ, Nutzungsdauer_WQ_dict[Wärmequelle], f_Inst_WQ, f_W_Insp_WQ,
                           Bedienaufwand_WQ, q, r, T)
    WGK_WQ_a = Annuität_WQ / Wärmemenge
...
def WGK_Gaskessel(P_max, Wärmemenge, Brennstoffbedarf, Brennstoffkosten, q, r, T, BEW="Nein"):
    ...
    Annuität = annuität(Investitionskosten, Nutzungsdauer, f_Inst, f_W_Insp, Bedienaufwand, q, r, T,
                        Brennstoffbedarf, Brennstoffkosten)
    WGK_a = Annuität / Wärmemenge
...
def WGK_Biomassekessel(Leistung_BMK, Wärmemenge, Brennstoffbedarf, Brennstoffkosten, q, r, T, BEW="Nein"):
    ...
    Annuität = annuität(Investitionskosten, Nutzungsdauer, f_Inst, f_W_Insp, Bedienaufwand, q, r, T, Brennstoffbedarf,
                        Brennstoffkosten)
    WGK_a = Annuität / Wärmemenge
...
def WGK_STA(Bruttofläche_STA, VS, typ, Wärmemenge, q, r, T, BEW="Nein"):
    if Wärmemenge == 0:
        return 0
    ...
    Annuität = annuität(Investitionskosten_Gesamt, Nutzungsdauer, f_Inst, f_W_Insp, Bedienaufwand, q, r, T)
    WGK_a = Annuität / Wärmemenge
    ...
    # Betriebskostenförderung_Gesamt = Betriebskostenförderung * Betriebskostenförderung_Dauer * Wärmemenge
    # Wärmepreis_BEW = (Gesamtkosten_Nutzungsdauer_BEW - Betriebskostenförderung_Gesamt) / Wärmemenge_Nutzungsdauer

    if BEW == "Nein":
        return WGK_a
    elif BEW == "Ja":
        print("BEW noch nicht implementiert. Daher Ausgabe der Wärmegestehungskosten ohne BEW.")
        return WGK_a
